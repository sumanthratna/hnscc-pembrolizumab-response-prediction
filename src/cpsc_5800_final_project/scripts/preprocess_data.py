...
def process_single_fov(args):
    ...
    try:
        ...
        return {"success": False, "sample_id": sample_id, "fov_id": fov_id, "error": str(e)}
...
def preprocess_data(
    ...
    for sample_id in sample_ids:
        ...
        for fov_id in fovs:
            tasks.append((
                sample_id, fov_id,
                str(marker_panel_path),
                bucket, prefix, output_dir
            ))
    ...
    patient_data = {sid: {"label": labels_dict.get(sid, 0), "fovs": []} for sid in sample_ids}
    completed = 0
    total_size_mb = 0
    cached_count = 0
    new_count = 0
            ...
            if result["success"]:
                ...
                if result.get("cached", False):
                    ...
                    if cached_count % 10 == 0:
                        print(f"  [{completed}/{len(tasks)}] ... {cached_count} cached files loaded")
                else:
                    new_count += 1
                    print(f"  [{completed}/{len(tasks)}] {result['sample_id']}_{result['fov_id']} - {result['size_mb']:.1f}MB (NEW)")
            else:
                print(f"  [{completed}/{len(tasks)}] {result['sample_id']}_{result['fov_id']} - FAILED")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-fovs", type=int, default=30,
                        help="Maximum FOVs per patient")
