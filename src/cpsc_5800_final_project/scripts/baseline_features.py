...
def main():
    ...
    for sample_id in sample_ids:
        ...
        fovs = enumerate_fovs(fs, bucket, sample_id, prefix)[
            : config["max_fovs_per_patient"]
        ]
        ...
        print(
            f"  {sample_id}: extracting from {len(fovs)} FOVs (label={'R' if label else 'NR'})"
        )
        ...
        if fov_features:
            ...
            print(
                f"    -> {len(fov_features)} FOVs, {len(patient_features[sample_id])} features"
            )
    ...
    print(
        f"  Original:  AUC = {auc:.4f}, Accuracy = {accuracy:.4f} ({sum(preds_binary == true_labels)}/{len(true_labels)})"
    )
    print(
        f"  Inverted:  AUC = {auc_inv:.4f}, Accuracy = {accuracy_inv:.4f} ({sum(inv_binary == true_labels)}/{len(true_labels)})"
    )
