...
def check_fov_alignment(
    ...
    # Load reference DAPI
    try:
        ref_dapi = load_dapi_image(
            fs, bucket, sample_id, fov_id, reference_cycle, prefix
        )
    ...
    return is_aligned, {
        ...
        "min_mi": (
            min(v for v in mi_values.values() if v is not None) if mi_values else None
        ),
    }
    ...
    for fov_id in fov_ids:
        ...
        if is_aligned:
            ...
            if verbose:
                ...
                print(
                    f"    Misaligned: {fov_id} (min MI: {min_mi:.4f})"
                    if min_mi
                    else f"    Misaligned: {fov_id}"
                )
